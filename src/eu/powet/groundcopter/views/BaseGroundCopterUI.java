...
public class BaseGroundCopterUI extends LinearLayout {
	...
	public void init_buttons(){
		...
		bt_read_mission.setText("Read Wps");
		bt_write_mission =new Button(ctx);
		bt_write_mission.setText("Write Wps");
		bt_clean_mission = new Button(ctx);
		bt_clean_mission.setText("Clear Wps");
	...
	public void init_map(){
		...
		MapTileProviderBasic tileProvider = new MapTileProviderBasic(  receiver, networkAvailablityCheck, TileSourceFactory.DEFAULT_TILE_SOURCE);
			...
			public void onLongPress(final MotionEvent e) {
				...
				ctx.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						alert.setMessage("Set Altitude delay");  
						LinearLayout l = new LinearLayout(ctx);
						// Set an EditText view to get user input   
						final EditText input_altitude = new EditText(ctx);

						l.addView(input_altitude); 
						input_altitude.setText("25");

						final EditText input_delay = new EditText(ctx);  
						input_delay.setText("1000");
						l.addView(input_delay);
						alert.setView(l); 
						alert.setPositiveButton("ok", new DialogInterface.OnClickListener() {  
							public void onClick(DialogInterface dialog, int whichButton) {  
								...
								int 	delay  = Integer.parseInt(input_delay.getText().toString()); 
								GeoPoint m = (GeoPoint) mapView.getProjection().fromPixels(e.getX(),e.getY());
								m.setAltitude(altitude);

								GeoPointMission wp = new GeoPointMission(m,delay);
